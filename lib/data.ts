...
/* ============================================================
   ...
   枠の並べ方は先月（7/26）と同じ。
     オープニング → コマ → 宿題タイム → デュエットタイム → コマ → 合唱 → 片付け
   今回は12:00〜17:40（5時間40分）と短いぶん、コマは3つ。区切り方はそのままで時刻だけが変わる。
   ...
   今回だけの決めごと：
     ...
     ・締めに合唱タイム。片付けの前に、全員でA室に戻って「世界で一つだけの花」を歌う。
       この10分は、デュエットタイムを50分から40分にして回した（12:00〜17:40の枠は動かせないため）。

   コマの分け方（9名を5人・4人の2部屋へ。決め方は先月と同じ考え方）：
     ①同席…36通りある2人の組み合わせが、3つのコマのどこかで必ず同じ部屋になる（36/36）。
       いちばん重なる組でも3コマとも同席、それ以外はどの組も1回だけ。
     ②各部屋に男女どちらもいる…女性だけ・男性だけの部屋を作らない。
       女性がひとりになる部屋は、コマごとに くる → すー → きい と入れ替わる。
     ③各部屋に役員（よっちゃん・くる・しゃちょー）が1人以上いる。
     ※宿題タイムとデュエットタイムは全員で1部屋に集まるので、コマで会えない相手はいない。
   ============================================================ */
...
export const karaokeRooms = {
  ...
  slots: [
    ...
    },
    {
      ...
      label: "コマ①",
      rooms: [
        { key: "A" as KaraokeRoomKey, members: ["よっちゃん", "Take", "ヒィ", "すー", "きい"] },
        { key: "B" as KaraokeRoomKey, members: ["くる", "しゃちょー", "シュウ", "ハッシー"] },
      ],
    },
    {
      id: "koma2",
    ...
    },
    {
      ...
      rooms: [
        { key: "A" as KaraokeRoomKey, members: ["しゃちょー", "Take", "ヒィ", "ハッシー", "きい"] },
        { key: "B" as KaraokeRoomKey, members: ["よっちゃん", "くる", "シュウ", "すー"] },
      ],
    },
    {
      id: "chorus",
  ...
  notes: [
    ...
    "この部屋割りは、どのコマでも歌う人数がかたよらないように、そして3つのコマのあいだに全員が一度は同じ部屋になるように組んでいます。",
  ],
};
