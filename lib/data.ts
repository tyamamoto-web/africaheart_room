...
export const nextEvent = {
  ...
  /* 参加受付の案内。recruitOpen で見た目が変わる（true=募っている琥珀色／false=落ち着いた青灰）。
     また募集するときは recruitOpen を true に戻し、下の2行を受付中の文言に差し替える。 */
  recruitOpen: false,
  recruitStatus: "受付終了",
  recruit:
    "参加の受付は締め切りました。ご参加のみなさま、当日お会いしましょう。回答の変更は、参加者アンケートから引き続きできます。",
};
...
/* ============================================================
   ...
   枠の並べ方（くるちゃんの提案：全員で集まるのは冒頭と合唱だけにして、
   あいだは全部コマにする。ミルユッテでは歌える回数が限られるので、
   カラオケでなるべく沢山歌えるようにするため）：
     オープニング・お誕生日会 → 自己紹介＋宿題（同時） → コマ①〜④ → 合唱 → 片付け
   宿題タイムは自己紹介と一緒にして冒頭の1時間にまとめ、デュエットタイムの枠は無くした。
   コマは4つ、どれも60分。12:00〜17:40（340分）にちょうど収まる。

   今回だけの決めごと：
     ・冒頭に、8月生まれのおふたり（よっちゃん・Take）のお誕生日会を置く。
       プレゼントを渡して、全員でハッピーバースデーを歌う。
     ・自己紹介は1人ずつ、そのまま宿題のお題で1曲歌って次の人へ回す（9人で60分）。
     ・締めに合唱タイム。片付けの前に、全員でA室に戻って「世界で一つだけの花」を歌う。

   コマの数を4つにした理由（同席の偏りをいちばん小さくできるため）：
     コマにできる240分を何等分するかで、同席回数の散らばりが変わる。
     すべての分け方を総当たりで調べた結果は次のとおり。
       4コマ(60分)…0回4組／1回4組／2回24組／3回4組　←採用。2回でそろう組が最も多い
       5コマ(48分)…1回10組／2回8組／3回18組　（全員1回以上になるが3回の組が18組に増える）
       6コマ(40分)…「全員2回以上」は0通り。さらに散らばる
     コマを増やすほど入れ替えが増えて歌える時間も減るので、4コマが両面でいちばん良い。
