...
// 役員専用・役員専用2は 9/6 にこのファイルから部品へ切り出した（TOP ＞ 設定 と共用）。
...
export default function AdminPage() {
  ...
  // 役員専用と役員専用2は合言葉を入れないと中身を出さない（合言葉の画面は OfficerGate が持つ）。
  // この2つと社長室の中身は、9/6 にこのファイルから部品へ切り出して TOP ＞ 設定 と共用にした。
  // 社長室はTOPページと同じ中身で、暗証番号は無い（9/6 に外した）。
  ...
  return (
    <main className="min-h-screen pb-16" style={{ background: "#ffffff" }}>
      ...
      {/* ── 役員専用タブ ── */}
      {/* 中身は app/components/OfficerPlan.tsx（9/6 にここから切り出した）。
          TOP ＞ 設定 ＞ 役員専用 にも同じものが出る。書いたものはどちらから開いても同じ。 */}
      {tab === "officer" && (
        <OfficerGate>
          <OfficerPlan />
        </OfficerGate>
      ...
      {/* ── 役員専用2タブ ── */}
      {/* 中身は app/components/OfficerRoleTable.tsx（9/6 にここから切り出した）。
          TOP ＞ 設定 ＞ 役員専用2 にも同じものが出る。 */}
      {tab === "officer2" && (
        <OfficerGate>
          <OfficerRoleTable />
        </OfficerGate>
